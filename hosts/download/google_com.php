...
{
    public $fNames = ['odt' => 'OpenDocument Text', 'docx' => 'Microsoft Word', 'rtf' => 'Rich Text Format', 'txt' => 'Plain Text', 'pdf' => 'PDF Document', 'epub' => 'EPUB Publication', 'zip' => 'Zipped html Document', 'pptx' => 'Microsoft PowerPoint', 'ods' => 'OpenDocument Spreadsheet', 'xlsx' => 'Microsoft Excel'];
    public $dFormats = ['odt', 'docx', 'rtf', 'txt', 'pdf', 'epub', 'zip'];
    public $pFormats = ['pptx', 'pdf'];
    public $ssFormats = ['ods', 'xlsx', 'pdf', 'zip'];
    public $sFormats = [13 => 'ods', 420 => 'xlsx', 12 => 'pdf'];

    public function Download($link)
    {
        if (!preg_match('@https?://(?:[\w\-]+\.)*(?:drive|docs)\.google\.com/(?:(?:folderview|open|(?:a/[\w\-\.]+/)?uc)\?(?:[\w\-\%]+=[\w\-\%]*&)*id=|(?:folder|file|document|presentation|spreadsheets)/d/|spreadsheet/ccc\?(?:[\w\-\%]+=[\w\-\%]*&)*key=|drive/folders/)([\w\-]{28,})@i', $link, $this->ID)) {
            html_error('File/Folder ID not found at link.');
        }
        $this->ID = $this->ID[1];

        // Use /open link for check if ID exists and also get it's type.
        $page = $this->GetPage('https://drive.google.com/open?id='.$this->ID);
        if (substr($page, 9, 3) == '404') {
            html_error('File/Folder doesn\'t exists.');
        }
        if (substr($page, 9, 1) != '7' || !preg_match('@\nLocation: https?://(?:[\w\-]+\.)*(?:drive|docs)\.google\.com/(?:drive/)?(\w+)[\?/]@i', $page, $type)) {
            html_error('Cannot find /open redirect.');
        }
    ...
    private function File()
    {
        ...
        $this->isPrivate($page);
        if (substr($page, 9, 1) != '2' || !preg_match('@\nLocation: (https?://(?:[\w\-]+\.)*googleusercontent\.com/[^\r\n]+)@i', $page, $dl)) {
            html_error('File\'s download-link not found.');
        }
    ...
    private function Document()
    {
        ...
        $this->isPrivate($page);
        if (empty($_GET['T8']['format']) && !isset($_GET['audl'])) {
            $this->formatSelector(1);
        }
    ...
    private function Spreadsheet()
    {
        ...
        if (substr($page, 9, 1) != '3' || !preg_match('@\nLocation: (https?://(?:[\w\-]+\.)*google\.com/[^\r\n]+)@i', $page, $redir)) {
            html_error('Redirect 1 not found');
        }
        $page = $this->GetPage($redir[1], $cookie);
        $cookie = GetCookiesArr($page, $cookie);
        if (empty($cookie['PREF'])) {
            html_error("Cookie 'PREF' not found");
        }

        $page = $this->GetPage("$url/ccc?key=".$this->ID, $cookie);

        if (empty($_GET['T8']['format']) && !isset($_GET['audl'])) {
            $this->formatSelector(4);
        }
        if (empty($_GET['T8']['format']) || ($fmcmd = array_search($_GET['T8']['format'], $this->sFormats)) === false) {
            reset($this->sFormats);
            $fmcmd = key($this->sFormats);
        }
        if (($cmdUrl = cut_str($page, '/fm?id=', '"')) == false || !preg_match('@[\w\-\.]+@i', $cmdUrl, $cmdId)) {
            html_error('Download ID not found.');
        }
    ...
    private function formatSelector($type = 1)
    {
        ...
        if (count($formats) == 1) {
            return $_GET['T8'] = ['format' => reset($formats)];
        }
        ...
        foreach ($formats as $ext) {
            echo "<option value='$ext'>".(!empty($this->fNames[$ext]) ? $this->fNames[$ext]." (.$ext)" : ".$ext")."</option>\n";
        }
        echo "</select>\n";
        $data = $this->DefaultParamArr('https://drive.google.com/open?id='.$this->ID);
        foreach ($data as $n => $v) {
            echo "<input type='hidden' name='$n' id='FS_$n' value='$v' />\n";
        }
    ...
    public function RedirectDownload($link, $FileName = 0, $cookie = 0, $post = 0, $referer = 0, $force_name = 0, $auth = 0, $addon = [])
    {
        $referer .= "\r\nRange: bytes=0-";
...
// [04-5-2019] Fixed redirection matching by NimaH79.
// [04-5-2019] Fixed spreadsheets by NimaH79.
